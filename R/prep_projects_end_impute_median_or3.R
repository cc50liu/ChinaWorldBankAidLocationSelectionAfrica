#prep_projects_end_impute_median_or3.R
################################################################################
...
rm(wb_proj, wb_locs, wb_geocoded, wb_proj_ancillary)
...
rm(ch_oda_df, ch_oda_p4_df,non_africa_isos_df)
...
#  - if no end dates for funder/sector to calc median, assume 3 year
...
oda_sect_group_end_df <- oda_sect_group_df %>% 
  ...
  mutate(all_na_end_years = all(is.na(end_year)),
         median_proj_years = ifelse(all_na_end_years,3,
...
oda_sect_group_end_df %>% 
  ...
  write.csv("./data/interim/oda_excluded_end_after_2016_impute_median_or3.csv")

#write file for use in further analysis
oda_sect_group_end_df %>% 
  filter(end_year <= 2016) %>% 
  write.csv("./data/interim/africa_oda_end_impute_median_or3.csv",row.names = FALSE)
