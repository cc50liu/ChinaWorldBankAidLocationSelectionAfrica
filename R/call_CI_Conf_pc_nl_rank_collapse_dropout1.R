...
if (treat_count < 100) {
  ...
  #input_df <- read.csv("./data/interim/input_pcnl_sh_co_d1_both_110.csv")
  ...
  if (nrow(input_df[!complete.cases(input_df),]) > 0) {
    ...
    ############################################################################   
    #ica_df <- read.csv("./results/test_runs/pcnl_sh_co_d1/ICA_both_110_pcnl_sh_co_d1_i2.csv")
    library(tidyr)   
    ...
    #print these to log
    print("Most likely treated:")
    print(most_likely_df %>% 
      select(rank,prW_est,col_index,treated,image_file,min_start_year))
    print("Least likely treated:")
    ...
    treat_control_map <- tm_shape(country_borders) +
      ...
      tm_symbols(size=.9,col=treat_color,shape=24) +
