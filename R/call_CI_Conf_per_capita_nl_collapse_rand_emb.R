...
#        Collapses time and uses a shallow 1-layer only neural network 
...
#initialize pc_nl_df and then loop over it, calculating per capita nightlights each year
pc_nl_df <- nl_df
...
dhs_conf_df <- read.csv("./data/interim/dhs_confounders.csv") %>% 
  select(-year)  #remove survey year column that could be confused with oda year
#n=9904

#join logged per capita nightlights to other confounder data
dhs_confounders_df <- pc_nl_log_df %>% 
  select(dhs_id, starts_with("log")) %>% 
  inner_join(dhs_conf_df, by="dhs_id")
...
#create a subset of dhs points with their iso codes for use in join below
...
var_order <- c("iwi_est_post_oda","log_pc_nl_pre_oda","log_avg_pop_dens",
               "log_avg_min_to_city",
               ...
               "landsat57",
               "landsat578")
var_labels <- c("Wealth (est, t+3)","Nightlights per capita (t-1,log)","Pop Density (t-1,log)",
...
# min_start_year max_end_year count proportion cntl_count
# <int>        <dbl> <int>      <dbl>      <dbl>
#   1           2001         2016   118    0.196          517
# 2           2002         2016   226    0.375          990
# 3           2003         2012     4    0.00663         18
# 4           2003         2016    86    0.143          377
# 5           2004         2011    85    0.141          372
# 6           2004         2016     8    0.0133          35
# 7           2005         2016    27    0.0448         118
# 8           2006         2016    23    0.0381         101
# 9           2008         2016     9    0.0149          39
# 10           2011         2016    13    0.0216          57
# 11           2012         2016     4    0.00663         18 
# dhs_c_df %>% 
#   group_by(min_start_year, max_end_year) %>% 
#   count()
...
if (treat_count < 100) {
  ...
  input_df <- obs_df %>% 
    select(dhs_id, country, iso3, lat, lon, treated, 
           min_start_year, max_end_year, image_file, iwi_est_post_oda,log_pc_nl_pre_oda,
           ...
           log_dist_km_to_gems,log_dist_km_to_dia,log_dist_km_to_petro,landsat57,landsat578) %>% 
    rename(cty = country) %>% 
  ...
  if (nrow(input_df[!complete.cases(input_df),]) > 0) {
    ...
    rm(dhs_conf_df,year_props, 
       dhs_c_df,dhs_confounders_df,
       dhs_ids_to_update,dhs_iso3_df,dhs_t_df,funder_sector_iso3,obs_df)
    ...
    long_input_df <- input_df %>%
      select(-dhs_id, -cty, -iso3, -lat, -lon, 
             -min_start_year, -max_end_year, -image_file) %>% 
    ...
    ridge_coeffs_df <- data.frame(funder_sector=fund_sect_param, 
                                  ...
                                  coefs=ridge_coeffs)
    write.csv(ridge_coeffs_df,
              paste0("./results/",fund_sect_param, "_", run,"_ridge_coef.csv"),
              row.names = FALSE)	  
