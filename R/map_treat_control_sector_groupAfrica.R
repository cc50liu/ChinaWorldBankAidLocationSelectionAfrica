...
sector_groups_df <- unique(rbind(
            wb_oda_df %>% filter(recipients_iso3 %in% africa_isos_df$iso3) %>% distinct(sector_group, sector_group_name),
            ch_oda_df %>% filter(recipients_iso3 %in% africa_isos_df$iso3) %>% distinct(sector_group, sector_group_name))) %>%
...
####################################################
sector_stats <- lapply(1:nrow(sector_groups_df), function(i) {
  sector <- sector_groups_df[i, "sector_group"]
  sector_name <- sector_groups_df[i, "sector_group_name"]
...
write.csv(sector_stats,"./data/interim/sector_group_treat_control_Africa.csv")
