# call_CI_Conf_dep3_pc_nl_rank_collapse.R
...
#        Collapses time and uses a shallow 3-layer only neural network 
...
if (treat_count < 100) {
  ...
  if (nrow(input_df[!complete.cases(input_df),]) > 0) {
    ...
    write.csv(treat_prob_log_df,
              paste0(results_dir,fund_sect_param, "_", run,"_treat_prob_log.csv"),
              row.names = FALSE)
    ...
    write.csv(ridge_coeffs_df,
              paste0(results_dir,fund_sect_param, "_", run,"_ridge_coef.csv"),
              row.names = FALSE)
