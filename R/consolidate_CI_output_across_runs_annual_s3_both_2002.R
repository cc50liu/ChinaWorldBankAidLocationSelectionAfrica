#consolidate_CI_output_across_runs_annual_s3_both_2002.R
...
#uncomment to test
run_versions <- c("tfrec_cnn_annual_s3_both_2002","tfrec_emb_annual_s3_both_2002")
run_directories <-c("./results/tfrec_cnn_annual_s3_both_2002",
                    "./results/tfrec_emb_annual_s3_both_2002")
run_shortnames <- c("cnn","emb")
group_label_for_filename <- "cnn_emb_annual_s3_both_2002"
group_label_for_titles <- "Annual Obs, treatments >=2002, both confounder variable"
...
var_order_all <- c("iwi_est_post_oda","log_pc_nl_pre_oda","log_avg_pop_dens",
                   ...
                   "landsat578","start_year","start_year_squared","treated_both_funders")
var_labels_all <- c("Wealth (est, t+3)","Nightlights per capita (t-1,log)","Pop Density (t-1,log)",
                    ...
                    "Landsat 5 & 7", "Landsat 5,7,& 8","Start Year","Start Year Squared","Treated Other Funder")
...
all_dif_salience_plot <- ggplot(compare_salience_se_df, 
  ...
  scale_color_manual(values = c("ch" = "indianred1", "wb" = "lightblue1", "both" = "blueviolet"),
                     breaks = c("ch","wb","both"),
                     labels = c("China", "World Bank", "Both")) +
  scale_shape_manual(values = c("cnn" = 15, "emb" = 21),
                     breaks = c("cnn","emb"),
                     labels = c("CNN","Emb")) +
  geom_vline(xintercept=0, color="gray80") +
