...
acquireImageRepFromDisk <- function(keys,training = F){
  ...
  if (treat_count < 100) {
    ...
    if (nrow(input_df[!complete.cases(input_df),]) > 0) {
      ...
      #trend variable (literal year in the X) 
      #add an indicator variable for the year here (handles baseline rate for the treatments)
      #or add year as a numeric covariate, year and year squared
