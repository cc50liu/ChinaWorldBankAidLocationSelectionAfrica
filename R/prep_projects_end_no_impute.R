################################################################################
# prep_projects_end_no_impute.R
# Load China and WB project data; filter to aid projects started in Africa 2001-2014;
# data cleanup and preparation
#
# Creates an end_year column, but does not impute it if missing.
# Does not exclude projects with end dates > 2016, since may  have effects 
# as current projects
################################################################################
library(dplyr)
library(sf)
...
################################################################################
# Create end_year column based on end_actual_isodate. 
################################################################################
oda_sect_group_end_df <- oda_sect_group_df %>% 
  mutate(end_year=as.integer(sub("^(\\d{4})-.*","\\1",end_actual_isodate))) 

################################################################################
# Do not exclude projects with known end year > 2016, but write them to file 
################################################################################
oda_sect_group_end_df %>% 
  filter(end_year > 2016) %>% 
  write.csv("./data/interim/oda_not_excluded_end_after_2016_noimpute.csv")

#write file for use in further analysis
oda_sect_group_end_df %>% 
write.csv("./data/interim/africa_oda_sector_group_end_noimpute.csv",row.names = FALSE)
