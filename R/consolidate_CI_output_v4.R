...
process_subdir <-  function(run_dir,sub_dir) {
  ...
  consolidated_dt <- consolidated_dt[,`:=`(sector = as.integer(sub(".*_(\\d+).*", "\\1", fund_sect_param)),
                                      funder = sub("(wb|ch|both).*", "\\1", fund_sect_param))]

  line_color <- ifelse(abs(consolidated_dt$LatitudeAnalysis.preDiffInLat1) > 
                         abs(consolidated_dt$LatitudeAnalysis.postDiffInLat1),
                       "lightgray", "black")

  difInLatPlot <- ggplot(consolidated_dt, aes(x = fund_sect_param)) +
    geom_segment(aes(x=reorder(fund_sect_param,sector),
                     xend=fund_sect_param,
    ...
    labs(
      ...
      subtitle = paste0("Latitude difference between treated and control groups","\n",
                        "Sectors:  ", ifelse(sub_dir=="BasicServices","Basic Services",sub_dir),
                        "          Run: ",run_version)) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Rotate x-axis labels by 45 degrees
  
  
  ggsave(paste0(run_directory,"/",sub_dir,"_",run_version,"_difInLat_Plot.pdf"),
         difInLatPlot,
  ...
  ##############################################################################  
  line_color <- ifelse(abs(consolidated_dt$LatitudeAnalysis.preDiffInLat2) > 
                         abs(consolidated_dt$LatitudeAnalysis.postDiffInLat2),
                       "lightgray", "black")
  
  difInLonPlot <- ggplot(consolidated_dt, aes(x = fund_sect_param)) +
    geom_segment(aes(x=reorder(fund_sect_param,sector),
                     xend=fund_sect_param,
    ...
    labs(
      ...
      color = "Funder",
      title = "Pre- and Post- Inverse Probability Weighting",
      subtitle = paste0("Longitude difference between treated and control groups","\n",
                        "Sectors:  ", ifelse(sub_dir=="BasicServices","Basic Services",sub_dir),
                        "          Run: ",run_version)) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Rotate x-axis labels by 45 degrees
  ...
  ggsave(paste0(run_directory,"/",sub_dir,"_",run_version,"_difInLon_Plot.pdf"),
         difInLonPlot,
         width=8, height = 6, dpi=300,
         bg="white", units="in")
  ...
  ##############################################################################  
  line_color <- ifelse(abs(consolidated_dt$ModelEvaluationMetrics.CELoss_out_baseline) > 
                         abs(consolidated_dt$ModelEvaluationMetrics.CELoss_out),
                       "lightgray", "black")
  
  difCELossPlot <- ggplot(consolidated_dt, aes(x = fund_sect_param)) +
    geom_segment(aes(x=reorder(fund_sect_param,sector),
                     xend=fund_sect_param,
    ...
    labs(
      ...
      subtitle = paste0("Sectors:  ", ifelse(sub_dir=="BasicServices","Basic Services",sub_dir),
                        "          Run: ",run_version)) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Rotate x-axis labels by 45 degrees
  
  
  ggsave(paste0(run_directory,"/",sub_dir,"_",run_version,"_dif_CE_Loss_Plot.pdf"),
         difCELossPlot,
         width=8, height = 6, dpi=300,
         bg="white", units="in")
  ...
  ##############################################################################  
  line_color <- ifelse(abs(consolidated_dt$ModelEvaluationMetrics.ClassError_out_baseline) > 
                         abs(consolidated_dt$ModelEvaluationMetrics.ClassError_out),
                       "lightgray", "black")
  
  dif_ClassError_plot <- ggplot(consolidated_dt, aes(x = fund_sect_param)) +
    geom_segment(aes(x=reorder(fund_sect_param,sector),
                     xend=fund_sect_param,
  ...
  ggsave(paste0(run_directory,"/",sub_dir,"_",run_version,"_dif_ClassError_Plot.pdf"),
         dif_ClassError_plot,
         width=8, height = 6, dpi=300,
         bg="white", units="in")
