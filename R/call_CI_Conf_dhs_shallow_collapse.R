# call_CI_Conf_dhs_shallow_collapse.R
...
if (treat_count < 100) {
  ...
  obs_df <- rbind(
    ...
    mutate(
      ...
      log_avg_nl_pre_oda = case_when(
        min_start_year %in% 2000:2001 ~ log_avg_nl_1996_1998,
        min_start_year %in% 2002:2004 ~ log_avg_nl_1999_2001,
        min_start_year %in% 2005:2007 ~ log_avg_nl_2002_2004,
        min_start_year %in% 2008:2010 ~ log_avg_nl_2005_2007,
        min_start_year %in% 2011:2013 ~ log_avg_nl_2008_2010,
        min_start_year %in% 2014:2016 ~ log_avg_nl_2011_2013),
  ...
  run_df <- obs_df %>% 
    left_join(country_confounders_df,
              by=join_by(iso3, min_start_year == year))

  #create input_df and write to file
  input_df <- run_df %>% 
    select(dhs_id, country, iso3, lat, lon, treated, 
           min_start_year, max_end_year, image_file, iwi_est_post_oda,
  ...
  if (nrow(input_df[!complete.cases(input_df),]) > 0) {
    ...
    rm(year_props, 
       country_confounders_df, dhs_c_df,dhs_confounders_df,
    ...
    ImageConfoundingAnalysis <- AnalyzeImageConfounding(
      ...
      keys = paste0(input_df$image_file,input_df$min_start_year), 
