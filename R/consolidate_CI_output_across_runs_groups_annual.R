#consolidate_CI_output_across_runs_group_annual.R
...
#uncomment to test
run_versions <- c("tfrec_emb_group_annual","tfrec_cnn_group_annual")
run_directories <-c("./results/tfrec_emb_group_annual",
...
get_matching_files <- function(run_version, run_directory) {
  #uncomment to test
   # run_version <- "tfrec_cnn_group_annual"
   # run_directory <- "./results/tfrec_cnn_group_annual"
  
  #filename_pattern <- paste0("ICA_",run_version,"_(wb|ch|both)_\\d{3}.csv")
  filename_pattern <- paste0("ICA_(wb|ch|both).*",run_version,".*.csv")
...
read_and_process_file <- function(file) {
  #uncomment to test
  #file <-  "./results/tfrec_emb_group_annual/ICA_both_PRO_tfrec_emb_group_annual_i1000.csv"
  
  #use a regular expression to construct ridge output file name corresponding
  #to the input file 
  ridge_file <- sub("(ICA_)(wb|ch|both)_([A-Z]*_)(.*)_i.*",
                    "\\2_\\3tab_conf_compare_\\4.csv", file)
...
outcome_df <- consolidated_dt %>% 
  rename(fund_sec_group = fund_sect_group_param) %>% 
  mutate(sector_group  = sub(".*_(.*)", "\\1", fund_sec_group),
         funder = sub("(wb|ch|both).*", "\\1", fund_sec_group),
         ...
         fund_sec_group_run = paste0(fund_sec_group,"_",run_short),
         fund_run = paste0(funder,"_",run_short)
  ) %>%
  arrange(fund_sec_group)  
...
sector_group_names_df <- read.csv("./data/interim/sector_group_names.csv")   %>% 
  ...
  distinct(sector_group, sector_group_name, sec_group_pre_name)
...
outcome_sector_group_df <- outcome_df %>%
  left_join(sector_group_names_df,join_by(sector_group==sector_group)) %>% 
  select(run,fund_sec_group,sec_group_pre_name,sector_group_name,treat_count,control_count,nTrainableParameters,
...
write.csv(outcome_sector_group_df,paste0("./results/ICA_xruns_all_",group_label_for_filename,".csv"),
          row.names=FALSE)
#outcome_sector_group_df <- read.csv("./results/ICA_xruns_all_cnn_emb_aggl.csv")
...
read_file_loc_probabilities <- function(file) {
  data <- fread(file, header = TRUE, select = grep("(prW_est|^run$|fund_sect_group_param)", colnames(fread(file))))
  return(data)
}
...
loc_probs_df <- loc_probs_dt %>% 
  ...
  left_join(sector_group_names_df,by="sector_group") %>% 
  select(run,fund_sec_group,sector_group, funder,run_short, fund_sec_group_run, sec_group_pre_name, starts_with("prW")) 

write.csv(loc_probs_df,paste0("./results/ICA_xruns_all_probs_",group_label_for_filename,".csv"),
          row.names=FALSE)

loc_probs_longer_df <- loc_probs_df %>% 
  select(fund_sec_group,run_short,starts_with("prW_est")) %>% 
  pivot_longer(cols=starts_with("prW_est")) %>% 
  pivot_wider(names_from=run_short) %>% 
  filter(complete.cases(.))

#write the correlations to a file
fund_sec_group_cor <- loc_probs_longer_df %>% 
  group_by(fund_sec_group) %>% 
  summarize(correlation=cor(cnn,emb)) %>% 
  arrange(correlation) 

  write.csv(fund_sec_group_cor,paste0("./results/corr_prob_xy_xruns_",group_label_for_filename,".csv"),
            row.names=FALSE)
  
  label_with_correlation <- function(fund_sec_group) {
    label <- fund_sec_group_cor %>%
      filter(fund_sec_group == fund_sec_group) %>%
      mutate(correlation = round(correlation,2)) %>% 
      pull(correlation)
    return(paste0(fund_sec_group," (corr ",label,")"))
  }
...
###########################################
outcome_sector_display_df <- outcome_sector_group_df %>% 
  select(sec_group_pre_name, run_short, funder, ate) %>% 
  pivot_wider(names_from=funder, values_from=ate) %>% 
  rename(Both=both,
         China=ch,
         World_Bank=wb,
         Run=run_short,
         SectorGroup=sec_group_pre_name
         ) 
...
#generate figures
###########################################
#ate by sector_group  funder and run 
ate_plot <- ggplot(outcome_sector_group_df,aes(x=tauHat_propensityHajek,
                                         y=fund_run,
                                         color=funder,
                                         shape=run_short)) +
...
##############################################################################
#create sector_group  and funder columns, order by sector
# consolidated_df <- outcome_sector_group_df %>% 
#   mutate(sector_group  = as.integer(sub(".*_(\\d+).*", "\\1", fund_sect_group_param)),
#          funder = sub("(wb|ch|both).*", "\\1", fund_sect_group_param),
#          run_short = run_shortnames[match(run,run_versions)],
#          fund_sec_group_run = paste0(fund_sect_group_param,"_",run_short)
#          ) %>% 
#   select(-starts_with("SGD_loss")) %>% 
#   arrange(sector) 

line_color <- ifelse(abs(outcome_sector_group_df$LatitudeAnalysis.preDiffInLat1) > 
                       abs(outcome_sector_group_df$LatitudeAnalysis.postDiffInLat1),
                     "gray92", "black")

difInLatPlot <- ggplot(outcome_sector_group_df, aes(x = fund_sec_group_run)) +
  geom_segment(aes(x=reorder(fund_sec_group_run,sector_group),
                   xend=fund_sec_group_run,
...
##############################################################################  
line_color <- ifelse(abs(outcome_sector_group_df$LatitudeAnalysis.preDiffInLat2) > 
                       abs(outcome_sector_group_df$LatitudeAnalysis.postDiffInLat2),
                     "gray92", "black")

difInLonPlot <- ggplot(outcome_sector_group_df, aes(x = fund_sec_group_run)) +
  geom_segment(aes(x=reorder(fund_sec_group_run,sector_group),
                   xend=fund_sec_group_run,
...
##############################################################################  
line_color <- ifelse(abs(outcome_sector_group_df$ModelEvaluationMetrics.CELoss_out_baseline) > 
                       abs(outcome_sector_group_df$ModelEvaluationMetrics.CELoss_out),
                     "gray92", "black")

difCELossPlot <- ggplot(outcome_sector_group_df, aes(x = fund_sec_group_run)) +
  geom_segment(aes(x=reorder(fund_sec_group_run,sector_group),
                   xend=fund_sec_group_run,
...
##############################################################################  
line_color <- ifelse(abs(outcome_sector_group_df$ModelEvaluationMetrics.ClassError_out_baseline) > 
                       abs(outcome_sector_group_df$ModelEvaluationMetrics.ClassError_out),
                     "gray92", "black")

dif_ClassError_plot <- ggplot(outcome_sector_group_df, aes(x = fund_sec_group_run)) +
  geom_segment(aes(x=reorder(fund_sec_group_run,sector_group),
                   xend=fund_sec_group_run,
...
compare_salience_df <- outcome_sector_group_df %>%
  select(funder, sec_group_pre_name, sector_group_name, run_short, starts_with("SalienceX"), starts_with("ridge_est")) %>% 
  pivot_longer(cols=-c(funder,sec_group_pre_name, sector_group_name, run_short)) %>% 
...
all_dif_salience_plot <- ggplot(compare_salience_se_df, 
                            aes(x=SalienceX, y=sector_group_name, shape=run_short, color=funder),
...
# geom_point(data=compare_salience_df,aes(x=ridge_est,y=sector_group_name, 
...
plot_tab_confounder <- function(term_var) {
  ...
  dif_salience_plot <- ggplot(compare_salience_se_df[compare_salience_se_df$term==term_var,], 
                                            aes(x=SalienceX, y=sec_group_pre_name, shape=run_short, color=funder),
                                            position=position_jitter(height = .25)) +
    geom_pointrange(aes(xmin=SalienceX - (SalienceX_se*1.96),
                        xmax=SalienceX + (SalienceX_se*1.96)),
                    position=position_jitter(height=0.25)) +
...
#to do: fix this ridge estimate    
    # geom_point(data=compare_salience_df,aes(x=ridge_est,y=sec_group_pre_name, 
    #                                         shape=run_short,fill="baseline"), color="gray80") +
    # scale_fill_manual(values=c("baseline"="gray80"),
    #                   labels=c("baseline"="No images")) +
    scale_color_manual(values = c("ch" = "indianred1", "wb" = "lightblue1", "both" = "blueviolet"),
